...
#define UNDER_LINE_SIZE CGSizeMake(160,2)

@implementation RCTabBar3
...
- (void)drawRect:(CGRect)rect
{
    ...
    for(int i = 0; i < [_itemArray count]; i++)
    {
        if(i != self.selectedIndex)
        {
            ...
            if([title length])
            {
                ...
                [title drawInRect:CGRectMake(i*width,15, width, self.bounds.size.height) withFont:[UIFont boldSystemFontOfSize:16] lineBreakMode:NSLineBreakByWordWrapping alignment:NSTextAlignmentCenter];
            }
        }
    }
    ...
    if([title length])
    {
        ...
        [title drawInRect:CGRectMake(self.selectedIndex*width, 15, width, self.bounds.size.height) withFont:[UIFont boldSystemFontOfSize:16] lineBreakMode:NSLineBreakByWordWrapping alignment:NSTextAlignmentCenter];
    }
}
...
- (void)updateContent
{
    ...
    [dict setObject:@"专辑" forKey:@"title"];
    [_itemArray addObject:dict];
    [dict release];
    
    dict = [[NSMutableDictionary alloc] init];
    [dict setObject:@"设计师履历" forKey:@"title"];
    [_itemArray addObject:dict];
    [dict release];
